      ...
      <section className="mx-auto mt-4 max-w-6xl space-y-6">
        ...
        <section className="card-standard p-6">
          ...
          <div className="grid gap-6">
            {rows.map((row) => (
              <div key={row.id} className="group relative rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:border-slate-300">
                <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-100">
                  <div className="flex items-center gap-5">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-black text-xl font-black text-white shadow-lg ring-4 ring-slate-50">
                      {row.tableNumber}
                    </div>
                    <div>
                      <h3 className="text-lg font-extrabold text-black tracking-tight">Table {row.tableNumber}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">UID</span>
                        <code className="text-[10px] font-mono font-medium text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                          {row.qrCode?.uniqueCode?.slice(0, 10).toUpperCase() || "PENDING"}
                        </code>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[10px] font-black uppercase tracking-widest shadow-sm ${
                        row.isActive 
                          ? "bg-emerald-500 text-white" 
                          : "bg-slate-200 text-slate-600"
                      }`}
                    >
                      <span className={`h-1.5 w-1.5 rounded-full ${row.isActive ? "bg-white animate-pulse" : "bg-slate-400"}`} />
                      {row.isActive ? "Online" : "Paused"}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-6 flex flex-wrap lg:flex-nowrap items-center gap-4">
                  <div className="flex flex-1 items-center gap-3">
                    <input
                      ...
                      className="input-standard min-w-[200px]"
                    ...
                    <button
                      ...
                      className={`min-w-[140px] ${row.isActive ? "btn-danger" : "btn-success"}`}
                    >
                      {row.isActive ? "Deactivate" : "Go Live"}
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2 rounded-2xl bg-slate-50 p-1">
                    <button
                      ...
                      className="btn-glass px-4 py-2 text-[10px] font-bold uppercase tracking-widest"
                      title="Rotate unique code"
                    >
                      Rotate
                    </button>
                    ...
                    <button
                      ...
                      className="btn-glass px-4 py-2 text-[10px] font-bold uppercase"
                    >
                      PNG
                    </button>
                    <button
